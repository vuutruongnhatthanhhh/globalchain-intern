...
const { EvaluateRequest, EndorseRequest, ProposedTransaction, PreparedTransaction, SubmitRequest, CommitStatusRequest, SignedCommitStatusRequest } = require('./gateway_pb');
...
const { ChannelHeader, Header, SignatureHeader } = require('./common/common_pb');
...
const App3: React.FC = () => {
  ...
  const handleEvaluateRequest = () => {
    ...
    const argsAsBytes = Array.of('CreateUser', ...(myArray ?? [])).map(asBytes)
    ...
    endorse.setChannelId('mychannel');
    endorse.setProposedTransaction(signedProposal);
    endorse.setEndorsingOrganizationsList(proposedTransaction.getEndorsingOrganizationsList());
    ...
    client.endorse(endorse, {})
      .then(response => {
        const txEnvelope = response.getPreparedTransaction();
        ...
        const digest2 = hashToUint8Array(payload)
        ...
        const getTransactionId = prepare.getTransactionId()

        const submitRequest = new SubmitRequest()
        submitRequest.setTransactionId(getTransactionId)
        submitRequest.setChannelId('mychannel')
        ...
        commitStatusRequest.setChannelId('mychannel')
        commitStatusRequest.setTransactionId(getTransactionId)
        commitStatusRequest.setIdentity(creator)

        const signedCommitStatusRequest = new SignedCommitStatusRequest()
        signedCommitStatusRequest.setRequest(commitStatusRequest.serializeBinary())

        const request = signedCommitStatusRequest.getRequest_asU8()
        const digest3 = hashToUint8Array(request)
        ...
        const response2 = client.commitStatus(signedCommitStatusRequest, {})
