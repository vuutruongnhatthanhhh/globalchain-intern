...
export PEER0_ORG1_CA=./test-network/organizations/peerOrganizations/org1.example.com/tlsca/tlsca.org1.example.com-cert.pem
export PEER0_ORG2_CA=./test-network/organizations/peerOrganizations/org2.example.com/tlsca/tlsca.org2.example.com-cert.pem
export PEER0_ORG3_CA=./test-network/organizations/peerOrganizations/org3.example.com/tlsca/tlsca.org3.example.com-cert.pem
...
setGlobals() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_MSPCONFIGPATH=./test-network/organizations/peerOrganizations/org3.example.com/users/Admin@org3.example.com/msp
...
packageChaincode() {
  ...
  address="${CC_NAME}:${CCAAS_SERVER_PORT}"
  prefix=$(basename "$0")
...
METADATA-EOF
  ...
  sleep 5
  cd ${PWD}/../blockchain
  META_INF_PATH=${PWD}/../chaincode/META-INF
    if [ -n "$META_INF_PATH" ]; then
     cp -a "$META_INF_PATH" "$tempdir/src/"
    fi
